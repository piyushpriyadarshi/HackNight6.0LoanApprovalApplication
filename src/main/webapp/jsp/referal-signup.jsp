...
<%--                    <form>--%>
                        <input type="hidden" id="referedBy" name="referedBy" value="${refer}" ng-model="createAccountForm.referedBy"/>
                        <div class="row">
...
<script>
    ...
    app.controller("CreateAccountController", function ($scope, $window,$http) {
        ...
        $scope.createAccountForm.referedBy="${refer}";

        $scope.submitForm=function(){
            $scope.createAccountForm=angular.toJson($scope.createAccountForm);
            console.log($scope.createAccountForm);
            $http({method : "POST",
                url : "${pageContext.request.contextPath}/referal",data:$scope.createAccountForm}).then(function(response){
